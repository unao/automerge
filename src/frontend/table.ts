...
import { OBJECT_ID, CONFLICTS } from './constants'
import { isObject } from '../common'
import { Context } from './context'

function compareRows (properties: any, row1: any, row2: any) {
...
export class Table {
   ...
   */
  filter (callback: (value: any, index: number, array: any[]) => any, thisArg: any) {
   ...
   */
  find (callback: (value: any, index: number, array: any[]) => any, thisArg: any) {
   ...
   */
  map (callback: (value: any, index: number, array: any[]) => any, thisArg: any) {
   ...
   */
  sort (arg: ((a: any, b: any) => number) | string | Array<string> | undefined) {
   ...
   */
  _clone () {
    if (!(this as any)[OBJECT_ID]) {
      throw new RangeError('clone() requires the objectId to be set')
    }
    return instantiateTable((this as any)[OBJECT_ID], Object.assign({}, this.entries))
  }

  /**
   * Sets the entry with key `id` to `value`.
   */
  set (id: ObjectId, value: any) {
...
class WriteableTable extends Table {
  ...
  add (row: any) {
    ...
    return this.context.addTableRow((this as any)[OBJECT_ID], row)
  }
  ...
  remove (id: ObjectId) {
    if (isObject(this.entries[id]) && this.entries[id][OBJECT_ID] === id) {
      this.context.deleteTableRow((this as any)[OBJECT_ID], id)
    } else {
 ...
 */
export function instantiateTable (objectId: ObjectId, entries?: any) {
